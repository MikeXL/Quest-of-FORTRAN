...
First things first, 
dated back to punch card days, 
first six characters are reserved for label, and 
important not to exeed column 72
...
_Function_ cannot be called from R, instead _subroutine_ can be called by *.Fortran*, 
this mechanism is motivated purely for taking advantage of Fortran numerical computation power.
Safe data types are _integer_, _double precision_, and _logical arguments_.
...
```
Then compile it with *R CMD SHLIB -c mc_pi_r.f*
